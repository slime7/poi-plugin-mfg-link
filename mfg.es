...
export const
  ...
  parseBattleResult = (battleResult, route) => {
    ...
    return ['/post/v1/battle_result', {
      _1: {
        ...
        lostFlag: battleResult.api_lost_flag ? battleResult.api_lost_flag.filter(x => x !== -1).map(x => !!x) : [],
  ...
  parseCreateitem = (data, material) => {
    ...
    let create = {
      ...
      steel: parseInt(material.api_item3),
      bauxite: parseInt(material.api_item4),
